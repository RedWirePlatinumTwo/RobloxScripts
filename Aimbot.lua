		...
		local AimbotFunction = game.RunService.RenderStepped:connect(function()
			...
			if not GameStats.IgnorePlayers then
					for i,v in pairs(GetFamily(m.Target)) do
						if game.Players:FindFirstChild(v.Name) and game.Players:FindFirstChild(v.Name).Character and math.floor(lplr:DistanceFromCharacter(game.Players:FindFirstChild(v.Name).Character.Humanoid.RootPart)) < GameStats.MaxStuds then
